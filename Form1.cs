...
namespace WindowsFormsApplication4
{
    public partial class Form1 : Form
    {
        ...
        double checkTable = 12.592; // 7 - 1 = 6 => m = 6, prob of crit = 0.05 => chi(6, 0.05) = 12.592;
        int size = 7;

        private void button1_Click(object sender, EventArgs e)
        {
            probT = new double[size]; // pi теоретические
            empericSeries = new double[size]; // pi эмпирические
            freq = new int[size]; // Частоты
            N = (int)tbN.Value;
            ...
            for (int i = 0; i < size; i++) {
                empericSeries[i] = freq[i] / (double)N; // pi = ni / N;
            ...
            mean = 0;
            for (int i = 0; i < size; i++)
                mean += freq[i] * probT[i]; // Теоретическое среднее pi * xi

            meanEmperic = 0;
            for (int i = 0; i < size; i++)
                meanEmperic += empericSeries[i] * freq[i]; // sum of ^pi*xi

            meanError = Math.Abs(meanEmperic - mean) / Math.Abs(mean); // Относительная погрешность среднего
            meanErr.Text = meanError.ToString("F2");
           
            variance = 0;
            for (int i = 0; i < size; i++) 
                variance += probT[i] * freq[i] * freq[i]; // Теоретическая дисперсия sum of (pi * xi * xi) - mean * mean
            variance -= mean * mean;

            varianceEmperic = 0;
            for (int i = 0; i < size; i++)
                varianceEmperic += empericSeries[i] * freq[i] * freq[i];
            varianceEmperic -= meanEmperic * meanEmperic; //sum of (^pi * xi^2) - meanEmp * meanEmp;

            varianceError = Math.Abs(varianceEmperic - variance) / Math.Abs(variance); // Относительная погрешность дисперсии
